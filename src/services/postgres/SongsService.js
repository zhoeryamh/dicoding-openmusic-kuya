...
class SongsService {
  ...
  async getSongs(request) {
    ...
    if ('title' in request) result = _.filter(result, (r) => r.title.toLowerCase().includes(request.title));
    if ('performer' in request)  result = _.filter(result, (r) => r.performer.toLowerCase().includes(request.performer));

    return result.map(mapSongToModel);
  }
